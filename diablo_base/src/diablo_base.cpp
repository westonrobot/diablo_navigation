...
int main(int argc, char** argv) {
  ...
  vehicle.telemetry->configTopic(DIABLO::OSDK::TOPIC_MOTOR,
                                 OSDK_PUSH_DATA_10Hz);
  ...
  while (ros::ok()) {
    ...
    broadcaster.sendTransform(tf::StampedTransform(
        tf::Transform(tf::Quaternion(quaternion.x(), quaternion.y(),
                                     quaternion.z(), quaternion.w()),
                      tf::Vector3(xcor, ycor, 0)),
    ...
    odom_data.twist.twist.linear.x =
        ((right_speed_1 + left_speed_1) / 2) * std::cos(theta);
